...
def render_knowledge_page():
    ...
    st.markdown(
        ...
        'Discover hidden connections in your text — entities, relationships, '
        'and concept networks extracted automatically using NLP.'
        '</p></div>',
    ...
    if sample_choice.startswith("📚 "):
        ...
        st.text_area(
            ...
            value=input_text,
            height=180,
            key="kg_text_display2",
            disabled=True,
        )
    else:
        input_text = st.text_area(
            "Paste your text here:",
            placeholder="Enter text with people, organizations, places, and events to build a knowledge graph...",
            height=180,
            key="kg_text_input",
        )
    ...
    # Entity type filter
    all_types = list(set(n.entity_type for n in graph.nodes))
    ...
    # Filter nodes
    filtered_nodes = [n for n in graph.nodes if n.entity_type in selected_types]
