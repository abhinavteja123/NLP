...
def render_analytics_page():
    ...
    st.markdown(
        ...
        "Insights from your Q&A sessions — question patterns, confidence trends, and more.</p>",
    ...
    # ── Top Entities ──
    render_section_header("🏷️ Top Entities Across All Answers")
    ...
    if all_entities:
        ...
        sorted_entities = sorted(entity_freq.items(), key=lambda x: x[1], reverse=True)[:15]

        entity_df = pd.DataFrame(sorted_entities, columns=["Entity", "Frequency"])
        ...
        st.info("No entities found in answers yet.")
